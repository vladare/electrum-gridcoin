...
class Exchanger(threading.Thread):
    ...
    def update_bv(self):
        ...
        quote_currencies = {}
        try:
            for r in jsonresp["Doge"]:
                quote_currencies[r] = Decimal(jsonresp["Doge"][r])
            with self.lock:
    ...
    def update_bf(self):
        ...
        for cur in quote_currencies:
            try:
                quote_currencies[cur] = self.get_json('api.bitfinex.com', "/v1/pubticker/doge" + cur.lower())["last_price"]
            except Exception:
                pass
        with self.lock:
            self.quote_currencies = quote_currencies
    ...
    def update_be(self):
        ...
        for cur in quote_currencies:
            try:
                quote_currencies[cur] = self.get_json('btc-e.com', "/api/2/doge_" + cur.lower() + "/ticker")["ticker"]["last"]
            except Exception:
                pass
        with self.lock:
            self.quote_currencies = quote_currencies
    ...
    def update_CNY(self):
        try:
            jsonresp = self.get_json('data.btcchina.com', "/data/ticker?market=dogecny")
        except Exception:
            return
    ...
    def update_ct(self):
        ...
        for cur in quote_currencies:
            try:
                quote_currencies[cur] = self.get_json('www.crypto-trade.com', "/api/1/ticker/doge_" + cur.lower())["data"]["last"]
            except Exception:
                pass
        with self.lock:
            self.quote_currencies = quote_currencies
    ...
    def update_kk(self):
        try:
            ...
            pairs = ','.join([k for k in resp_currencies if k.startswith("XDogeZ")])
    ...
    def update_ok(self):
        try:
            jsonresp = self.get_json('www.okcoin.com', "/api/ticker.do?symbol=doge_cny")
        except Exception:
            return
        quote_currencies = {"CNY": 0.0}
        cnyprice = jsonresp["ticker"]["last"]
    ...
    def update_vs(self):
        ...
        for cur in quote_currencies:
            try:
                quote_currencies[cur] = self.get_json('api.vaultofsatoshi.com', "/public/ticker?order_currency=Doge&payment_currency=" + cur)["data"]["closing_price"]["value"]
            except Exception:
                pass
        with self.lock:
            self.quote_currencies = quote_currencies
    ...
    def history_tab_update(self):
        if self.config.get('history_rates', 'unchecked') == "checked":
            ...
            if cur_exchange == "CoinDesk":
                ...
                if cur_currency == "VEF":
                    try:
                        resp_hist = self.exchanger.get_json('api.bitcoinvenezuela.com', "/historical/index.php")['VEF_Doge']
                    except Exception:
                        return
                elif cur_currency == "ARS":
                    try:
                        resp_hist = self.exchanger.get_json('api.bitcoinvenezuela.com', "/historical/index.php")['ARS_Doge']
                    except Exception:
                        return
                else:
                    return
