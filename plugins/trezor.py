...
class TrezorWallet(NewWallet):
    ...
    def address_id(self, address):
        ...
        return "44'/30'/%s'/%d/%d" % (account_id, change, address_index)

    def create_main_account(self, password):
        self.create_account('Main account', None) #name, empty password

    def derive_xkeys(self, root, derivation, password):
        derivation = derivation.replace(self.root_name,"44'/30'/")
    ...
    def get_master_public_key(self):
        if not self.mpk:
            self.mpk = self.get_public_key("44'/30'")
        return self.mpk
    ...
    def sign_message(self, address, message, password):
        ...
        try:
            ...
            give_error(e)
        try:
            msg_sig = self.get_client().sign_message('Dogecoin', address_n, message)
        except Exception, e:
            give_error(e)
        finally:
            twd.emit(SIGNAL('trezor_done'))
    ...
    def sign_transaction(self, tx, keypairs, password):
        ...
        try:
            signed_tx = self.get_client().sign_tx('Dogecoin', inputs, outputs)[1]
        except Exception, e:
            give_error(e)
        finally:
            twd.emit(SIGNAL('trezor_done'))
    ...
    def check_proper_device(self):
        ...
        if not self.device_checked:
            ...
            device_address = self.get_client().get_address('Dogecoin', n)
